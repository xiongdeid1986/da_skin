            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        $query = "SELECT count(*) FROM tbltickets WHERE date>'" . $filterdate . "' AND ( email='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$email) . "'";
                        if( $userid )
                        {
...
function closeInactiveTickets()
{
    ...
    if( 0 < $whmcs->get_config('CloseInactiveTickets') )
    {
        ...
        for( $result = full_query($query); $data = mysqli_fetch_array($result); $i++ )
        {
            $id = $data['id'];
            $did = $data['did'];
            $subject = $data['title'];
