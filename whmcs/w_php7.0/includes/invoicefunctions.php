...
function refundInvoicePayment($transid, $amount, $sendtogateway, $addascredit = '', $sendemail = true, $refundtransid = '')
{
    ...
    if( $sendtogateway )
    {
        ...
        if( $gatewayModule->functionExists('refund') )
        {
            ...
            $gatewayname = $gateways->getDisplayName($gateway);
...
function processPaidInvoice($invoiceid, $noemail = '', $date = '')
{
    ...
    if( WHMCS_Invoices::issequentialpaidinvoicenumberingenabled() )
    {
        ...
        {
            update_query('tblinvoices', array( 'invoicenum' => WHMCS_Invoices::getnextsequentialpaidinvoicenumber() ), array( 'id' => $invoiceid ));
        }
    }
    ...
    $result = select_query('tblinvoiceitems', '', "invoiceid='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$invoiceid) . "' AND type!=''", 'id', 'ASC');
    while( $data = mysqli_fetch_array($result) )
    {
        $userid = $data['userid'];
        $type = $data['type'];
...
function makeHostingPayment($func_domainid)
{
    ...
    while( $data = mysqli_fetch_array($result) )
    {
        ...
        $addonResult = select_query('tbladdons', 'autoactivate,welcomeemail', array( 'id' => $addonid ));
        $data = mysqli_fetch_array($addonResult);
        ...
        if( $autoactivate )
        {
            ...
            if( $welcomeemail )
            {
                $welcomeResult = select_query('tblemailtemplates', 'name', array( 'id' => $welcomeemail ));
                $data = mysqli_fetch_array($welcomeResult);
...
{
    $result = select_query('tblhostingaddons', 'id,hostingid,addonid,billingcycle,status,nextduedate', array( 'id' => $func_addonid ));
    $data = mysqli_fetch_array($result);
    $id = $data['id'];
    ...
    if( $status == 'Pending' )
    {
        ...
        if( $autoactivate )
        {
            ...
            if( $welcomeemail )
            {
                ...
                $data = mysqli_fetch_array($result);
                $welcomeemailname = $data['name'];
                sendMessage($welcomeemailname, $serviceid);
            }
    ...
    if( $status == 'Suspended' )
    {
        ...
        if( $addonid )
        {
            ...
            if( $suspendproduct )
            {
                $result2 = select_query('tblhosting', 'servertype', array( "tblhosting.id" => $serviceid ), '', '', '', "tblproducts ON tblproducts.id=tblhosting.packageid");
                $data2 = mysqli_fetch_array($result2);
                $module = $data2[0];
                ...
                $serverresult = ServerUnsuspendAccount($serviceid);
            }
        }
