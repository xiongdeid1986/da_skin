...
else
{
    ...
    if( $a == 'starttimer' )
    {
        ...
        while( $activetimersdata = simulate_fetch_assoc($activetimers) )
        {
                        ...
                        else
                        {
                            ...
                            if( $a == 'gettimesheet' )
                            {
                                ...
                                if( project_management_checkperm("Bill Tasks") )
                                {
                                    ...
                                    $dti = 0;
                                    for( $tasksresult = select_query('mod_projecttasks', 'id,task', array( 'projectid' => $projectid, 'billed' => '0' )); $tasksdata = simulate_fetch_assoc($tasksresult); $dti++ )
                                    {
...
function project_management_timesoutput($vars, $taskid)
{
    ...
    while( $timerdata = simulate_fetch_assoc($result2) )
    {
        $show_startresume = 'false';
        $timerid = $timerdata['id'];
        $timeradmin = simulate_fetch_assoc(select_query('tbladmins', 'firstname,lastname', array( 'id' => $timerdata['adminid'] )));
