...
function bulkpricingupdater_output($vars)
{
    ...
    if( $validate || $doupdate )
    {
        ...
        if( $doupdate )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        if( $type == 'domains' )
                        {
                            ...
                            if( in_array('dnsmanagement', $domainaddons) )
                            {
                                $query .= " AND (dnsmanagement='1' OR dnsmanagement='on')";
                            }
                            else
                            {
                            ...
                            if( in_array('emailforwarding', $domainaddons) )
                            {
                                $query .= " AND (emailforwarding='1' OR emailforwarding='on')";
                            }
                            else
                            {
                            ...
                            if( in_array('idprotection', $domainaddons) )
                            {
                                $query .= " AND (idprotection='1' OR idprotection='on')";
                            }
                            else
                            {
                ...
                if( $result )
                {
                    $resultsResponse = "Success!<br />" . mysqli_affected_rows() . " Database Record(s) Updated";
                }
                else
                {
    ...
    echo "</select></td></tr>\n</table>\n\n        </div>\n        <div id=\"criteria-domains\">\n\n            <h2>Domains Criteria</h2>\n\n<table class=\"form\" width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"3\">\n<tr><td width=\"20%\" class=\"fieldlabel\">TLD(s)</td><td class=\"fieldarea\">";
    ...
    echo "</td></tr>\n<tr><td class=\"fieldlabel\">Registration Period</td><td class=\"fieldarea\"><select name=\"regperiod\">\n";
    for( $domainyears = 1; $domainyears <= 10; $domainyears++ )
    {
        echo "<option value=\"" . $domainyears . "\">" . $domainyears . " Year" . (1 < $domainyears ? 's' : '') . "</option>";
    }
    echo "</select></td></tr>\n<tr><td class=\"fieldlabel\">Status</td><td class=\"fieldarea\"><select name=\"domainstatus[]\" size=\"8\" multiple=\"true\">\n<option selected>Pending</option>\n<option selected>Pending Transfer</option>\n<option selected>Active</option>\n<option>Terminated</option>\n<option>Cancelled</option>\n<option>Expired</option>\n<option>Fraud</option>\n</select></td></tr>\n<tr><td class=\"fieldlabel\">Domain Addons</td><td class=\"fieldarea\">\n    <label><input type=\"checkbox\" name=\"domainaddons[]\" value=\"dnsmanagement\" /> DNS Management</label>\n    <label><input type=\"checkbox\" name=\"domainaddons[]\" value=\"emailforwarding\" /> Email Forwarding</label>\n    <label><input type=\"checkbox\" name=\"domainaddons[]\" value=\"idprotection\" /> ID Protection</label>\n</td></tr>\n<tr><td class=\"fieldlabel\">Current Price</td><td class=\"fieldarea\"><input type=\"text\" name=\"domaincurrentprice\" size=\"10\" value=\"\" /> (Optional)</td></tr>\n<tr><td class=\"fieldlabel\">Currency</td><td class=\"fieldarea\"><select name=\"domaincurrid\">";
    $result = select_query('tblcurrencies', 'id,code', '', 'code', 'ASC');
    while( $data = mysqli_fetch_array($result) )
    {
        echo "<option value=\"" . $data['id'] . "\"";
