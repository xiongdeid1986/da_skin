 ...
 * @return int
 */
function addClient($firstname, $lastname, $companyname, $email, $address1, $address2, $city, $state, $postcode, $country, $phonenumber, $password, $securityqid = '', $securityqans = '', $sendemail = 'on', $additionalData = array(  ))
{
    global $whmcs;
    global $remote_ip;
    if( !$country )
    ...
    saveCustomFields($uid, $customFields, 'client');
    if( $sendemail )
    {
...
function getSecurityQuestions($questionid = '')
{
    ...
    $results = array(  );
    while( $data = simulate_fetch_assoc($query) )
    {
...
function generateClientPW($plain, $salt = '', $ignoreconfig = false)
{
    ...
    else
    {
        ...
        $pw = md5($salt . WHMCS_Input_Sanitize::decode($plain)) . ":" . $salt;
    }
    return $pw;
}
    ...
    if( $username && ($password || $_SESSION['adminid'] || $twofadone) )
    {
        ...
        if( $login_uid )
        {
            ...
            if( $password === $check_pwd || isset($_SESSION['adminid']) && $adminallowedclientlogin || $loginsharematch || $twofadone )
            {
                ...
                if( !isset($_SESSION['adminid']) )
                {
                    update_query('tblclients', array( 'lastlogin' => "now()", 'ip' => WHMCS_Utility_Environment_CurrentUser::getip(), 'host' => WHMCS_Utility_Environment_CurrentUser::getiphost(), 'pwresetkey' => '', 'pwresetexpiry' => 0 ), array( 'id' => $login_uid ));
                }
                ...
                WHMCS_Session::set('upw', generateClientLoginHash($login_uid, $login_cid, $login_pwd));
                if( !isset($_SESSION['adminid']) )
                {
...
function generateClientLoginHash($userId, $contactId, $passwordHash)
{
    ...
    $userIp = $whmcs->get_config('DisableSessionIPCheck') ? '' : WHMCS_Utility_Environment_CurrentUser::getip();
...
{
    $gatewayclass = new WHMCS_Gateways();
