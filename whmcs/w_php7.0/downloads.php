...
if( $action == 'displaycat' )
{
    ...
    $i = 1;
    for( $result = select_query('tbldownloadcats', '', array( 'parentid' => $catid, 'hidden' => '' ), 'name', 'ASC'); $data = mysqli_fetch_array($result); $i++ )
    {
        $idkb = $data['id'];
...
else
{
    ...
    else
    {
        ...
        $i = 1;
        for( $result = select_query('tbldownloadcats', '', array( 'parentid' => '0', 'hidden' => '' ), 'name', 'ASC'); $data = mysqli_fetch_array($result); $i++ )
        {
            $idkb = $data['id'];
            $dlcats[$i] = array( 'id' => $idkb, 'name' => $data['name'], 'urlfriendlyname' => getModRewriteFriendlyString($data['name']), 'description' => $data['description'] );
            $idnumbers = array(  );
