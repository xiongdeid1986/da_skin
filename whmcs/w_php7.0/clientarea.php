...
if( $action == '' )
{
    ...
    $result = select_query('tbltickets', '', "userid='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$client->getID()) . "' AND status IN (" . db_build_in_array($statusfilter) . ")", 'lastreply', 'DESC');
    while( $data = mysqli_fetch_array($result) )
    {
        $id = $data['id'];
        $tid = $data['tid'];
        ...
        else
        {
            ...
            else
            {
                if( $action == 'creditcard' )
                {
                    ...
                    while( $data = mysqli_fetch_array($result) )
                    {
                        $gatewaymod = $data['gateway'];
                        $gateway = new WHMCS_Module_Gateway();
                        if( $gateway->load($gatewaymod) && $gateway->functionExists('remoteupdate') )
                        {
                            $gateway->loadSettings();
                            $extraParams = array( 'gatewayid' => get_query_val('tblclients', 'gatewayid', array( 'id' => $client->getID() )) );
                            $remoteupdatecode = $gateway->call('remoteupdate', $extraParams);
                            if( !$remoteupdatecode )
                            {
                    ...
                    if( $submit )
                    {
                        ...
                        if( !$errormessage )
                        {
                            redir("action=creditcard&savesuccess=1");
                        }
                    }
                    if( $whmcs->get_req_var('remove') && $whmcs->get_config('CCAllowCustomerDelete') )
                    {
                        check_token();
                        updateCCDetails($client->getID(), '', '', '', '', '');
                        redir("action=creditcard&deletesuccess=1");
                    }
                    if( $whmcs->get_req_var('savesuccess') )
                    {
                        $smartyvalues['successful'] = true;
                    }
                    if( $whmcs->get_req_var('deletesuccess') )
                    {
                        $smartyvalues['successful'] = true;
                        $smartyvalues['deletecc'] = true;
                    }
                    $smartyvalues['errormessage'] = $errormessage;
                ...
                else
                {
                    if( $action == 'changepw' )
                    {
                        ...
                        {
                            check_token();
                            $existingpw = WHMCS_Input_Sanitize::decode($existingpw);
                            $newpw = WHMCS_Input_Sanitize::decode($newpw);
                            $confirmpw = WHMCS_Input_Sanitize::decode($confirmpw);
                            $userId = $client->getID();
                            $contactId = (int) WHMCS_Session::get('cid');
                            if( $contactId )
                            {
                            ...
                            {
                                $passwordToSave = generateClientPW($newpw);
                                if( $contactId )
                                {
                                ...
                                WHMCS_Session::set('upw', generateClientLoginHash($userId, $contactId, $passwordToSave));
                                ...
                                if( $action == 'productdetails' )
                                {
                                    ...
                                    if( $service->getModule() && $service->getData('status') == 'Active' )
                                    {
                                        ...
                                        if( $service->hasFunction('ChangePassword') && $service->getAllowChangePassword() )
                                        {
                                            ...
                                            if( $modulechangepassword )
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    $updatearr = array( 'password' => WHMCS_Input_Sanitize::decode($newpassword1) );
                                                    $success = $service->moduleCall('ChangePassword', $updatearr);
                                                    if( $success )
                                                    {
                                                        logActivity("Module Change Password Successful - Service ID: " . $id);
                                                        run_hook('AfterModuleChangePassword', array( 'params' => $service->buildParams($updatearr) ));
                                        ...
                                        if( checkContactPermission('manageproducts', true) )
                                        {
                                            ...
                                            {
                                                $module = new WHMCS_Module_Server();
                                                $module->loadByServiceID($service->getID());
                                                $moduleparams = $module->buildParams();
                                                ...
                                                $moduleparams = $module->prepareParams($moduleparams);
                                ...
                                else
                                {
                                    if( $action == 'domains' )
                                    {
                                        ...
                                        while( $data = mysqli_fetch_array($result) )
                                        {
                                            ...
                                            $manageNS = false;
                                            if( $domains->hasFunction('SaveNameservers') )
                                            {
                                                $manageNS = true;
                                            }
                                            ...
                                            $storedDomains[] = array( 'id' => $id, 'domain' => $domain, 'amount' => formatCurrency($amount), 'registrationdate' => $registrationdate, 'nextduedate' => $nextduedate, 'expirydate' => $expirydate, 'status' => $status, 'rawstatus' => $rawstatus, 'statustext' => $_LANG['clientarea' . $rawstatus], 'autorenew' => $autorenew, 'managens' => $manageNS );
                                        }
                                    ...
                                    else
                                    {
                                        if( $action == 'domaindetails' )
                                        {
                                            ...
                                            $allowrenew = false;
                                            if( $whmcs->get_config('EnableDomainRenewalOrders') && ($domainstatus == 'Active' || $domainstatus == 'Expired') )
                                            {
                                                $allowrenew = true;
                                            }
                                            $autorenew = $donotrenew ? false : true;
                                                ...
                                                if( $modop == 'custom' && in_array($a, $allowedclientregistrarfunctions) )
                                                {
                                                    ...
                                                    if( is_array($data) )
                                                    {
                                                        if( isset($data['templatefile']) )
                                                        {
                                                            ...
                                                            {
                                                                throw new WHMCS_Exception_Fatal("Invalid Registrar Module Name");
                                                            }
                                                            if( !isValidforPath($data['templatefile']) )
                                                            {
                                                                throw new WHMCS_Exception_Fatal("Invalid Template Filename");
                                                            }
                                            ...
                                            if( $action == 'domaincontacts' )
                                            {
                                                ...
                                                if( $sub == 'save' )
                                                {
                                                    ...
                                                    foreach( $wc as $wc_key => $wc_val )
                                                    {
                                                        if( $wc_val == 'contact' )
                                                        {
                                                            ...
                                                            {
                                                                $client = new WHMCS_Client($client->getID());
                                                ...
                                                if( $success )
                                                {
                                                    ...
                                                    foreach( $contactDetails as $contactType )
                                                    {
                                                        ...
                                                        {
                                                            if( isset($_LANG['domaincontactdetails'][$contactFieldName]) )
                                                            {
                                                                $contactTranslations[$contactFieldName] = $_LANG['domaincontactdetails'][$contactFieldName];
                                                            }
                                                            else
                                                            {
                                                    ...
                                                    $smartyvalues['contactdetails'] = $contactDetails;
                                                                    ...
                                                                    else
                                                                    {
                                                                        if( $action == 'emails' )
                                                                        {
                                                                            ...
                                                                            while( $data = mysqli_fetch_array($result) )
                                                                            {
                                                                                ...
                                                                                $date = fromMySQLDate($date, 1, 1);
                                                                                $emails[] = array( 'id' => (int) $id, 'date' => WHMCS_Input_Sanitize::makesafeforoutput($date), 'subject' => WHMCS_Input_Sanitize::makesafeforoutput($subject) );
                                                                            }
                                                                            ...
                                                                            else
                                                                            {
                                                                                ...
                                                                                else
                                                                                {
                                                                                    if( $action == 'masspay' )
                                                                                    {
                                                                                        ...
                                                                                        {
                                                                                            $invoiceids = array(  );
                                                                                            $result = full_query("SELECT id FROM tblinvoices WHERE userid = " . $client->getID() . " AND status='Unpaid' AND (select count(id) from tblinvoiceitems where invoiceid=tblinvoices.id and type='Invoice')<=0 ORDER BY id DESC");
                                                                                            while( $data = mysqli_fetch_array($result) )
                                                                                            {
                                                                                                $invoiceids[] = $data['id'];
                                                                                            }
                                                                                        ...
                                                                                        if( $geninvoice )
                                                                                        {
                                                                                            ...
                                                                                            require("includes/processinvoices.php");
                                                                                            $invoiceid = createInvoices($client->getID(), true, true, array( 'invoices' => $invoiceids ));
                                                                                            $invoiceid = (int) $invoiceid;
                                                                                            $paymentmethod = WHMCS_Gateways::makesafename($paymentmethod);
                                                                                            $result = select_query('tblpaymentgateways', 'value', array( 'gateway' => $paymentmethod, 'setting' => 'type' ));
                                                                                            $data = mysqli_fetch_array($result);
                                                                                                                ...
                                                                                                                if( $save )
                                                                                                                {
                                                                                                                    ...
                                                                                                                    foreach( $wc as $wc_key => $wc_val )
                                                                                                                    {
                                                                                                                        if( $wc_val == 'contact' )
                                                                                                                        {
                                                                                                                            ...
                                                                                                                            $tmpContactDetails = $client->getDetails($selcid);
                                                                                                                            $contactdetails[$wc_key] = $domains->buildWHOISSaveArray($tmpContactDetails);
                                                                                                                        }
                                                                                                                    }
                                                                                                                    foreach( $domainids as $domainid )
