...
function update_query($table, $array, $where, $resource = null)
{
    ...
    if( is_array($array) && 0 < count($array) )
    {
        ...
        if( !$result && ($CONFIG['SQLErrorReporting'] || $mysqli_errors) )
        {
            logActivity("SQL Error: " . mysqli_error($resource) . " - Full Query: " . $query);
        }
        $query_count++;
    }
    else
...
function insert_query($table, $array, $resource = null)
{
    ...
    $result = full_query($query, $resource);
    if( !$result && $table != 'tblactivitylog' && ($CONFIG['SQLErrorReporting'] || $mysqli_errors) )
    {
        logActivity("SQL Error: " . mysqli_error($resource) . " - Full Query: " . $query);
    }
    $query_count++;
    $id = mysqli_insert_id($resource);
    return $id;
}
...
function full_query($query, $userHandle = null)
{
    ...
    $result = mysqli_query($GLOBALS['whmcsmysql'],$query, $handle);
    if( !$result && ($CONFIG['SQLErrorReporting'] || $mysqli_errors) )
    {
        logActivity("SQL Error: " . mysqli_error($handle) . " - Full Query: " . $query);
    }
...
{
    $string = mysqli_real_escape_string($GLOBALS['whmcsmysql'],$string);
    return $string;
}
