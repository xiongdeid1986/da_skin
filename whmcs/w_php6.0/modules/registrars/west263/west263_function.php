...
{
	$cn = iconv('UTF-8','GBK',$cn);
	$ChineseSpell = new ChineseSpell();
	if(checkChinese($cn))
	{
		$pinyin = $ChineseSpell->getFullSpell($cn);
	}
	else
	{
		$pinyin = $ChineseSpell->getFullSpell($cn,'');
...
class ChineseSpell
{
     ...
     * @return string
     */
    public function getFullSpell($chinese, $delimiter = ' ')
    {
        $spell = $this->getChineseSpells($chinese);
		foreach($spell as $val){
			if($val)
				$str .= ucfirst($val) . $delimiter;
		}
		$str = substr($str, 0, -1);
        return $str;
    }
     ...
     * @return string
     */
    private function getChineseSpells($chinese)
    {
        $result = array();
