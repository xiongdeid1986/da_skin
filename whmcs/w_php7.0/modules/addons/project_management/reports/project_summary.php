...
for( $result = select_query('mod_project', '', "duedate>='" . toMySQLDate($datefrom) . "' AND duedate<='" . toMySQLDate($dateto) . "'" . $adminquery . $statusquery); $data = mysqli_fetch_array($result); $i++ )
{
    $totaltaskstime = 0;
    $projectid = $data['id'];
    ...
    while( $data2 = simulate_fetch_assoc($timerresult) )
    {
        $rowcount = $rowtotal = 0;
        $taskid = $data2['id'];
        $task = $data2['task'];
