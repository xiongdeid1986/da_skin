...
function dmf_config() {
    return array(
        ...
        'FriendlyName' => array(
            'Type' => 'System',
            'Value' => '支付宝当面付 - 发卡君（www.fakajun.com）',
        ),
 ...
 * @return [type]         [description]
 */
function dmf_link($params)
{
    if (!stristr($_SERVER['PHP_SELF'], 'viewinvoice')) {
        /** 帐单页面 */
        return '<img style="width:150px;" src="/modules/gateways/dmf/dmf.png" alt="支付宝当面付" />';
    }
    ...
    $http['method']        =   'alipay.trade.precreate'; //提交方法（操作指令）
    $http['nonce']         =   time(); //随即字符串（时间戳）
    /**
     * 业务参数 JSON格式提交
     */
    $http['biz_content']  =   json_encode([
        'out_trade_no'  =>  'zuishuaideren'.$params['invoiceid'],
    ...
    if (isset($_GET['query'])) {
        $http['method']    =   'alipay.trade.query'; //提交方法（操作指令）
        $http['sign']      =     sign($http);
        $res = json_decode(result($http), true);
        /** 支付成功 */
        if ($res['code'] == 10000) {
