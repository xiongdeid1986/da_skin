...
<div id="topnav">
  <div id="welcome">{$_ADMINLANG.global.welcomeback} <strong>{$admin_username}</strong>&nbsp;&nbsp;[ <a href="#" onclick="toggleadvsearch();return false"><strong>{$_ADMINLANG.global.advancedsearch}</strong></a> | <a href="myaccount.php"><strong>{$_ADMINLANG.global.myaccount}</strong></a> | <a href="logout.php"><strong>{$_ADMINLANG.global.logout}</strong></a> ]{$topBarNotification}</div>
