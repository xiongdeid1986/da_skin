...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'dl') {
	...
	if($adminid) {
		...
		if($userid) {
			$accessallowed = get_query_val('mod_project', 'id', array(
				'id' => $projectid,
				'userid' => $userid
			));
			if(!$accessallowed) {
	...
	if($msg) {
		...
		$result = select_query('mod_projectmessages', 'id,attachments', array(
			'id' => $msg,
			'projectid' => $projectid
		));
		$data   = mysqli_fetch_array($result);
		...
		$result    = select_query('mod_project', 'id,attachments', array(
			'id' => $projectid
		));
		$data      = mysqli_fetch_array($result);
		$projectid = $data['id'];
...
function project_management_config() {
	$configarray = array(
		...
		'author' => 'WHMCS',
		'language' => 'english',
		'description' => "Track & manage projects, tasks & time with ease using the Official Project Management Addon for WHMCS.<br />Find out more & purchase @ <a href=\"http://nullrefer.com/?https://www.whmcs.com/addons/project-management\" target=\"_blank\">www.whmcs.com/addons/project-management</a>",
		'premium' => true,
		'fields' => array()
	);
	if(!PMADDONLICENSE) {
		$configarray['fields']['license'] = array(
			'FriendlyName' => "License Check Failed",
			'Type' => '',
			'Description' => "You need to purchase the project management addon from <a href=\"http://nullrefer.com/?https://www.whmcs.com/addons/project-management\" target=\"_blank\">www.whmcs.com/addons/project-management</a> before you can use this functionality. If you just purchased it recently, please <a href=\"configaddonmods.php?pmrefresh=1#project_management\">click here</a> to refresh this message"
		);
	}
	$fieldname = "Master Admin Users";
	$result    = select_query('tbladminroles', '', '', 'name', 'ASC');
	while($data = mysqli_fetch_array($result)) {
		$configarray['fields']['masteradmin' . $data['id']] = array(
			'FriendlyName' => $fieldname,
			'Type' => 'yesno',
			'Description' => "Allow Access to Settings for <strong>" . $data['name'] . "</strong> users"
		);
		$fieldname                                          = '';
	}
	return $configarray;
...
You can find out more about it and purchase @ <a href=\"http://nullrefer.com/?https://www.whmcs.com/addons/project-management\" target=\"_blank\">www.whmcs.com/addons/project-management</a><br /><br />
If you have only recently purchased the addon, please <a href=\"addonmodules.php?module=project_management&refresh=1\">click here</a> to perform a license refresh.
	...
	if($_REQUEST['createproj']) {
		...
		if($_REQUEST['ajax']) {
			if(project_management_checkperm("Create New Projects")) {
				...
				echo "<tr><td><a href=\"addonmodules.php?module=project_management&m=view&projectid=" . $projectid . "\">" . $projectid . "</a></td><td><a href=\"addonmodules.php?module=project_management&m=view&projectid=" . $projectid . "\">" . $insertarr['title'] . "</a> <span id=\"projecttimercontrol" . $projectid . "\" class=\"tickettimer\"><a href=\"#\" onclick=\"projectstarttimer('" . $projectid . "');return false\"><img src=\"../modules/addons/project_management/images/starttimer.png\" align=\"absmiddle\" border=\"0\" /> Start Tracking Time</a></td><td>" . get_query_val('tbladmins', "CONCAT(firstname,' ',lastname)", array(
					'id' => $insertarr['adminid']
				)) . "</td><td>" . $dates['created'] . "</td><td>" . $dates['duedate'] . "</td><td>" . getTodaysDate() . "</td><td>" . $statuses[0] . "</td></tr>";
				exit( dirname(__FILE__) . " | line".__LINE__ );
			}
			echo '0';
			exit( dirname(__FILE__) . " | line".__LINE__ );
		...
		if(project_management_checkperm("Create New Projects") && trim($_REQUEST['title'])) {
			$projectid = insert_query('mod_project', array(
				'title' => $_REQUEST['title'],
				'userid' => $_REQUEST['userid'],
				'created' => toMySQLDate($_REQUEST['created']),
				'duedate' => toMySQLDate($_REQUEST['duedate']),
				'adminid' => $_REQUEST['adminid'],
				'ticketids' => $_REQUEST['ticketnum'],
				'status' => $statuses[0]
			));
...
<select class=\"title\" name=\"adminid\">";
		...
		$result = select_query('tbladmins', 'id,firstname,lastname', array(
			'disabled' => '0'
...
<div align=\"center\"><input type=\"submit\" value=\"" . $vars['_lang']['create'] . "\" class=\"create\" />&nbsp;<input type=\"button\" value=\"" . $vars['_lang']['cancel'] . "\" class=\"create\" onclick=\"cancelnewproject();return false\" /></div>
	...
	$headeroutput .= "<div class=\"adminbar\"><a href=\"" . $modulelink . "\"><img src=\"images/icons/system.png\" /> " . $vars['_lang']['home'] . "</a> <a href=\"" . $modulelink . "&m=reports\"><img src=\"images/icons/reports.png\" /> " . $vars['_lang']['viewreports'] . "</a> <a href=\"reports.php?moduletype=addons&modulename=project_management&subdir=reports&report=project_staff_logs\"><img src=\"images/icons/billableitems.png\" /> " . $vars['_lang']['viewstafflogs'] . "</a> <a href=\"" . $modulelink . "&m=activity\"><img src=\"images/icons/logs.png\" /> " . $vars['_lang']['viewactivitylogs'] . "</a> ";
	...
	$headeroutput .= "<a href=\"http://nullrefer.com/?http://docs.whmcs.com/Project_Management\"><img src=\"images/icons/support.png\" /> " . $vars['_lang']['help'] . "</a></div>
...
function project_management_tasksstatus($projectid, $vars) {
	$totaltasks = get_query_val('mod_projecttasks', "COUNT(id)", array(
		'projectid' => $projectid
	));
	$completed  = get_query_val('mod_projecttasks', "COUNT(id)", array(
		'projectid' => $projectid,
		'completed' => '1'
	));
...
function project_management_log($projectid, $msg) {
	insert_query('mod_projectlog', array(
		'projectid' => $projectid,
		'date' => "now()",
		'msg' => $msg,
		'adminid' => $_SESSION['adminid']
	));
	update_query('mod_project', array(
		'lastmodified' => "now()"
	), array(
		'id' => $projectid
	));
...
function project_management_checkperm($perm) {
	...
	static $PMPerms;
	if(!$PMPerms) {
		$perms   = get_query_val('tbladdonmodules', 'value', array(
			'module' => 'project_management',
			'setting' => 'perms'
		));
	...
	if(!$PMRoleID) {
		$PMRoleID = get_query_val('tbladmins', 'roleid', array(
			'id' => $_SESSION['adminid']
		));
	}
	$revperms    = array();
...
function project_management_permslist() {
	$permissions = array(
		"Create New Projects",
		"View All Projects",
		'13' => "View Only Assigned Projects",
...
function project_management_check_viewproject($projectid, $adminid = '') {
	...
	$projectid = get_query_val('mod_project', 'id', array(
		'id' => $projectid
	));
	if(!$projectid) {
		return false;
	...
	if(project_management_checkperm("View Only Assigned Projects")) {
		$projectadminid = get_query_val('mod_project', 'adminid', array(
			'id' => $projectid
		));
		if($adminid == $projectadminid) {
			return true;
		}
		$tasksresult = select_query('mod_projecttasks', 'adminid', array(
			'projectid' => $projectid
		));
		while($tasksdata = simulate_fetch_assoc($tasksresult)) {
...
function project_management_check_masteradmin($PMRoleID = '', $adminid = '') {
	if(!$PMRoleID) {
		$PMRoleID = get_query_val('tbladmins', 'roleid', array(
			'id' => $adminid ? $adminid : $_SESSION['adminid']
		));
	}
	if(get_query_val('tbladdonmodules', 'value', array(
		'module' => 'project_management',
		'setting' => 'masteradmin' . $PMRoleID
	)) == 'on') {
		return true;
	}
	return false;
