...
add_hook('ClientAreaPage', 1, 'newtlds_hook_clientareavars');
...
function newtlds__hook_DB_GetCreateHookTable()
{
    ...
    if( !mysqli_num_rows(full_query("select * from `tblconfiguration` where setting='newtlds__cronbatchsize';")) )
    {
...
{
    global $newtlds__CronDBName;
    if( !mysqli_num_rows(full_query("SHOW TABLES LIKE '" . $newtlds__CronDBName . "'")) )
    {
        return false;
    }
    return true;
...
function newtlds__hook_DB_GetWatchlistSettingsLocal()
{
    global $newtlds__hook_DefaultEnvironment;
    global $newtlds__DBName;
    $result = select_query($newtlds__DBName, 'enabled,configured,portalid,environment,enomlogin,enompassword,companyname,companyurl', array(  ));
    $data = mysqli_fetch_array($result);
...
{
    if( !mysqli_num_rows(full_query("select domain from tbldomains where domain='" . $domain . "'")) )
    {
        return false;
    }
    return true;
...
function newtlds_hook_clientareavars()
{
    return array( 'enomnewtldsenabled' => true, 'enomnewtldslink' => "index.php?m=newtlds" );
}
