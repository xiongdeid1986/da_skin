...
class WHMCS_Database implements WHMCS_Config_DatabaseInterface
{
    ...
    public function legacyCreateDatabaseConnection()
    {
        ...
        $charset = $this->getDatabaseCharset();
        global $whmcsmysql;
        $whmcsmysql = @mysqli_connect($host, $username, $password,$dbname);
        $selected_db = @mysqli_select_db($whmcsmysql,$dbname);
        if( empty($GLOBALS['whmcsmysql']) ){$GLOBALS['whmcsmysql'] = $whmcsmysql;}
        if( !$selected_db )
        {
