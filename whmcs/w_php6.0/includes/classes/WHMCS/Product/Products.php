...
class WHMCS_Product_Products
{
    ...
    public function getProducts($groupId = null)
    {
        ...
        $products = array(  );
        $result = select_query('tblproducts', "tblproducts.id, tblproducts.gid, tblproducts.name, tblproducts.servertype," . "tblproducts.retired, tblproductgroups.name AS groupname", $where, "tblproductgroups`.`order` ASC, `tblproducts`.`order` ASC, `name", 'ASC', '', "tblproductgroups ON tblproducts.gid=tblproductgroups.id");
