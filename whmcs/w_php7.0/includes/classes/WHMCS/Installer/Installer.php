...
class WHMCS_Installer_Installer {
	...
	public function checkIfInstalled() {
		if(file_exists(ROOTDIR . "/configuration.php")) {
			$db_host = $db_username = $db_password = $db_name = $mysqli_charset = $customadminpath = '';
			include(ROOTDIR . "/configuration.php");
			if($customadminpath) {
				$this->customadminpath = $customadminpath;
			}
			if(function_exists('mysqli_connect') && $db_username && $db_name) {
				try {
					$this->setDatabase($this->factoryDatabase($db_host, $db_username, $db_password, $db_name, $mysqli_charset));
				}
				catch(Exception $e) {
				}
				$db = $this->getDatabase();
	 ...
	 * @param string $mysqli_charset
	 *
	 * @return resource
	 * @throws Exception
	 */
	public function factoryDatabase($db_host = "127.0.0.1", $db_username = '', $db_password = '', $db_name = '', $mysqli_charset = '') {
		...
		if($mysqli_charset) {
			$query = sprintf("SET NAMES \"%s\"", mysqli_real_escape_string($GLOBALS['whmcsmysql'],$mysqli_charset));
			mysqli_query($link ,$query, $link);
		}
    	if( empty($GLOBALS['whmcsmysql']) ){$GLOBALS['whmcsmysql'] = $link;}
		return $link;
	}
	/**
	...
	protected function fetchDatabaseConfigurationValue($key = 'Version') {
		...
		$db    = $this->getDatabase();
		if($result = mysqli_query($GLOBALS['whmcsmysql'],$query, $db)) {
	...
	protected function storeDatabaseConfigurationValue($value, $key = 'Version') {
		...
		mysqli_query($GLOBALS['whmcsmysql'],$query, $db);
		return $this;
	}
	/**
			...
			if($this->shouldRunUpgrade($versionOfInterest)) {
				if(function_exists('v' . $vernum . 'Upgrade')) {
					...
					if(file_exists($installerSqlDir . $filename)) {
						mysqli_import_file($filename, $installerSqlDir);
					}
				}
