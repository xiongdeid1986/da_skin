...
<li><a id="Menu-Support" {if in_array("Support Center Overview",$admin_perms)}href="supportcenter.php"{/if} title="Support">{$_ADMINLANG.support.title}</a>
  <ul>
    ...
    {if in_array("List Support Tickets",$admin_perms)}<li class="expand"><a id="Menu-Support-Support_Tickets" href="supporttickets.php">{$_ADMINLANG.support.supporttickets}</a>
        ...
        {foreach from=$menuticketstatuses item=status}
            <li><a id="Menu-Support-Support_Tickets-{$status.title|replace:' ':'_'}" href="supporttickets.php?view={$status.title}">- {$status.title}</a></li>
        {/foreach}
        </ul>
    </li>{/if}
