...
function newtlds_activate($vars)
{
    global $newtlds_DefaultEnvironment;
    global $newtlds_DBName;
    $LANG = $vars['_lang'];
    $sql = newtlds_DB_GetCreateTable();
    $retval = mysqli_query($GLOBALS['whmcsmysql'],$sql);
    if( !$retval )
    {
        return array( 'status' => 'error', 'description' => $LANG['activate_failed1'] . $newtlds_DBName . " : " . mysqli_error($GLOBALS['whmcsmysql'], );
    }
    $companyname = '';
    $domain = '';
...
function newtlds_deactivate($vars)
{
    ...
    if( !$retval )
    {
        return array( 'status' => 'error', 'description' => $LANG['deactivate_failed2'] . ":  " . mysqli_error($GLOBALS['whmcsmysql'], );
    }
...
function newtlds_clientarea($vars)
{
    ...
    if( $userid )
    {
        $query = mysqli_query($GLOBALS['whmcsmysql'],"SELECT email FROM tblclients WHERE id=" . (int) $userid) or exit( "There was a problem with the SQL query: " . mysqli_error($GLOBALS['whmcsmysql'], );
...
function newtlds_DB_GetCreateHookTable()
{
    ...
    if( !mysqli_num_rows(full_query("select * from `tblconfiguration` where setting='newtlds_cronbatchsize';")) )
    {
...
{
    if( !mysqli_num_rows(full_query("SHOW TABLES LIKE '" . $newtlds_DBName . "'")) )
    {
        return false;
    }
    return true;
...
{
    global $newtlds_CronDBName;
    if( !mysqli_num_rows(full_query("SHOW TABLES LIKE '" . $newtlds_CronDBName . "'")) )
    {
        return false;
    }
    return true;
...
function newtlds_output($vars)
{
    ...
    echo "</strong> <span style=\"color:red\">*</span>\n\t\t\t\t\t\t\t</td>\n\t\t\t\t\t\t\t<td width=\"50%\"style=\"text-align:right\">\n\t\t\t\t\t\t\t\t<a href=\"http://nullrefer.com/?https://www.whmcs.com/members/freeenomaccount.php\" target=\"_blank\"\">";
