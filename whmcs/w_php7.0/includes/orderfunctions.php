...
function getPricingInfo($pid, $inclconfigops = false, $upgrade = false)
{
    ...
    $configoptions = new WHMCS_Product_ConfigOptions();
        ...
        foreach( $_SESSION['cart']['products'] as $key => $productdata )
        {
            ...
            if( $checkout )
            {
                ...
                for( $qtycount = 1; $qtycount <= $qty; $qtycount++ )
                {
                    ...
                    if( $stockcontrol )
                    {
                        full_query("UPDATE tblproducts SET qty=qty-1 WHERE id='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$pid) . "'");
                    }
                    if( $configoptionsdb )
                    {
            ...
            if( $CONFIG['TaxEnabled'] && $tax && !$clientsdetails['taxexempt'] )
            {
                ...
                if( $CONFIG['TaxType'] == 'Inclusive' )
                {
                    ...
                    $productdata['pricing']['totaltoday'] = $product_tax / $totaltaxrates;
                    $total_without_tax = $productdata['pricing']['totaltoday'];
    ...
    if( array_key_exists('domains', $_SESSION['cart']) && is_array($_SESSION['cart']['domains']) )
    {
        ...
        foreach( $_SESSION['cart']['domains'] as $key => $domain )
        {
            ...
            if( !isset($temppricelist[$regperiod][$domaintype]) )
            {
                ...
                throw new WHMCS_Exception_Fatal($errMsg);
            }
            ...
            $hookret = run_hook('OrderDomainPricingOverride', array( 'type' => $domaintype, 'domain' => $domainname, 'regperiod' => $regperiod, 'dnsmanagement' => $domain['dnsmanagement'], 'emailforwarding' => $domain['emailforwarding'], 'idprotection' => $domain['idprotection'], 'eppcode' => WHMCS_Input_Sanitize::decode($domain['eppcode']) ));
            foreach( $hookret as $hookret2 )
            {
                if( strlen($hookret2) )
                {
...
function getAvailableOrderPaymentGateways()
{
    ...
    foreach( $gatewayslist as $module => $vals )
    {
        if( $vals['type'] == 'CC' || $vals['type'] == 'OfflineCC' )
        {
            if( !isValidforPath($module) )
            {
                $errMsg = "Invalid Gateway Module Name";
                if( $whmcs->isApiRequest() )
                {
                    $apiresults = array( 'result' => 'error', 'message' => $errMsg );
                    return $apiresults;
