...
<div class="tc-tabsbar center-tabs arrow">
	<ul class="nav nav-tabs">
		...
		{if $condlinks.domaintrans}<li class="active"><a href="domainchecker.php?search=bulktransfer">{$LANG.domainbulktransfersearch}</a></li>{/if}
		{/if}
	</ul>
</div>
...
<div class="tab-content padding-16 text-center">
	<form method="post" action="domainchecker.php" class="form-horizontal">
		...
		<div class="text-center">
			<textarea name="bulkdomains" rows="8" class="input-xxlarge" placeholder="You can transfer your existing domains to us today.  To get started, simply enter the domains below, one per line - do not include the www. or http://">{$bulkdomains}</textarea>
		</div>
	{if $capatacha}
	...
	<div class="space-6"></div>
		<input type="submit" onclick="$('#modalpleasewait').modal();" value="{$LANG.domainstransfer}" class="btn btn-success">
	</form>
</div>
	...
	<div class="col-sm-10 col-sm-offset-1">
		<form method="post" action="{$systemsslurl}cart.php" class="form-horizontal">
						...
						<td>{$result.domain}
							...
							{if $result.status eq "unavailable"}
                                ...
                                <ul class="list-unstyled visible-xs">
									<li><select name="domainsregperiod[{$result.domain}]">{foreach key=period item=regoption from=$result.regoptions}{if $regoption.transfer}<option value="{$period}">{$period} {$LANG.orderyears} @ {$regoption.transfer}</option>{/if}{/foreach}</select></li>
								</ul>
							{/if}
						</td>
						<td class="col-small center">
			...
			<div class="padding-all text-center">
				<input type="submit" value="{$LANG.ordernowbutton} &raquo;" class="btn btn-success btn-sm">
			</div>
