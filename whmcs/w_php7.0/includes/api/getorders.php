...
if( $id )
{
    $where[] = "o.id=" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$id);
}
if( $userid )
{
    $where[] = "o.userid=" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$userid);
}
if( $status )
{
    $where[] = "o.status='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$status) . "'";
}
if( count($where) )
{
...
while( $orderdata = simulate_fetch_assoc($result) )
{
    ...
    if( $renewals )
    {
        ...
        foreach( $renewals as $renewal )
        {
            ...
            $data = mysqli_fetch_array($renewalResult);
            $domainid = $data['id'];
            $type = $data['type'];
            $domain = $data['domain'];
