...
else
{
    if( $a == 'view' )
    {
        ...
        $i = 1;
        $tasks = array(  );
        for( $result = select_query('mod_projecttasks', 'id,task,notes,adminid,created,duedate,completed', array( 'projectid' => $projectid ), 'order', 'ASC'); $data = simulate_fetch_assoc($result); $i++ )
        {
            $tasks[$i] = $data;
