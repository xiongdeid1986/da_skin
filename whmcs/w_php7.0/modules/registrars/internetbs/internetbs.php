...
function internetbs_chekPhone($phoneNumber)
{
    ...
    $phoneNumber = str_replace("\t", '', $phoneNumber);
    if( preg_match("^\\+[0-9]{1,4}\\.[0-9 ]+\$", $phoneNumber) )
    {
        return true;
    }
    return false;
...
function internetbs_get_utf8_parameters($params)
{
    ...
    $result = full_query("SELECT orderid FROM tbldomains WHERE id='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$params['domainid']) . "' LIMIT 1;");
    if( !($row = mysqli_fetch_array($result, MYSQL_ASSOC)) )
    {
        return $params;
    }
    $result = full_query("SELECT userid,contactid FROM tblorders WHERE id='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$row['orderid']) . "' LIMIT 1;");
    if( !($row = mysqli_fetch_array($result, MYSQL_ASSOC)) )
    {
        return $params;
    }
    if( $row['contactid'] )
    {
        $result = full_query("SELECT firstname, lastname, companyname, email, address1, address2, city, state, postcode, country, phonenumber FROM tblcontacts WHERE id='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$row['contactid']) . "' LIMIT 1;");
        if( !($row = mysqli_fetch_array($result, MYSQL_ASSOC)) )
        {
            return $params;
        }
    ...
    else
    {
        if( $row['userid'] )
        {
            $result = full_query("SELECT firstname, lastname, companyname, email, address1, address2, city, state, postcode, country, phonenumber FROM tblclients WHERE id='" . mysqli_real_escape_string($GLOBALS['whmcsmysql'],$row['userid']) . "' LIMIT 1;");
            if( !($row = mysqli_fetch_array($result, MYSQL_ASSOC)) )
            {
                return $params;
            }
